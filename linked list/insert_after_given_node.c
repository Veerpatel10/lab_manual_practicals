...
struct Node * insert_after_given_node(struct Node *head,struct Node *given,int data)
{
    struct Node * ptr = (struct Node *)malloc(sizeof(struct Node));
    ptr -> data = data;
    ptr -> next = given -> next;
...
int main()
{
    ...
    printf("\n-----------Data after insertion after given node-------- \n");
    head = insert_after_given_node(head,second,12);
    linklisttrav(head);
    return 0;
}
